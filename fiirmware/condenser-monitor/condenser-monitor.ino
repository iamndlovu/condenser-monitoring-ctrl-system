...
char serverAddress[] = "196.220.120.138";  // server address # check on list of connected device in hotspotting phone!!
int port = 5000;
...
void loop() {
  ...
  //PH Function
  float getpH() {
  int sensorValue = readSmoothedSensorValue2();//analogRead(pHpin);  
       //Serial.print("Sensor Value:");
            //Serial.println(sensorValue);            //read analog value from  pH sesnor
         float voltage = sensorValue * 5.00/4095;
         //Serial.print("Voltage:");
        
         float ampVolt =voltage - offset;
          //Serial.println(ampVolt);
            /*float ampVolt = 5*voltage;
            float pHValue = ampVolt/5 *14;*/
 float pHValue = (-m * ampVolt) + c;// map(sensorValue, 0, 4095, 14, 0);  // Map analog value to pH range (0-14)

  //Serial.print("pH Value:  ");
  //Serial.println(pHValue);  // prints ph value to 2 dp
...
int readSmoothedSensorValue2(){
     int total = 0;
  for (int i = 0; i < averagePoints; i++) {
    int sensorValue2 = analogRead(pHpin);
